...
MLFLOW_EXPERIMENT = os.getenv("MLFLOW_EXPERIMENT_NAME", "clinical_translation")
MLFLOW_TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", "mlruns")
EVAL_DATASET = os.getenv("EVAL_DATASET", "medlane")
...
def run_pipeline():
    """Execute the full clinical translation pipeline."""
    logger.info("=" * 60)
    logger.info("Clinical Translation Pipeline — Starting")
    logger.info("=" * 60)
    ...
    with mlflow.start_run(run_name="clinical_translation_run") as run:
        ...
        mlflow.log_params(
            {
                ...
                "graph_nodes": "extract_cause → extract_location → extract_treatment",
            }
        )
        ...
        for i, sample in enumerate(test_data):
            ...
            try:
                output = graph.invoke(
                    {
                        "clinical_input": sample["clinical"],
                        "cause": "",
                        "location": "",
                        "treatment": "",
                    }
                ...
                results.append(
                    {
                        "clinical_input": sample["clinical"],
                        "reference_simple": sample.get("simple", ""),
        ...
        valid_results = [r for r in results if not r["cause"].startswith("[ERROR")]
