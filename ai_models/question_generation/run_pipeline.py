...
# ---------------------------------------------------------------------------
MODEL_NAME = os.getenv("MODEL_NAME", "medgemma-27b-text-latest")
TEMPERATURE = float(os.getenv("TEMPERATURE", "0.4"))
MAX_OUTPUT_TOKENS = int(os.getenv("MAX_OUTPUT_TOKENS", "2048"))
MLFLOW_EXPERIMENT = os.getenv("MLFLOW_EXPERIMENT_NAME", "question_generation")
MLFLOW_TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", "mlruns")
EVAL_DATASET = os.getenv("EVAL_DATASET", "medquad")
EVAL_N_SAMPLES = int(os.getenv("EVAL_N_SAMPLES", "50"))
...
def run_pipeline():
    ...
    logger.info("=" * 60)
    logger.info("Question Generation Pipeline — Starting")
    logger.info("=" * 60)
    ...
    data = ingest(source=EVAL_DATASET, n_samples=EVAL_N_SAMPLES)
    ...
    with mlflow.start_run(run_name="question_generation_run") as run:
        ...
        mlflow.log_params(
            {
                ...
                "graph_nodes": (
                    "generate_understanding → generate_treatment"
                    " → generate_lifestyle"
                ),
            }
        )
        ...
        for i, sample in enumerate(test_data):
            ...
            try:
                # MedQuAD clinical_text is the medical report content.
                # For evaluation, health_data and symptoms are set to
                # "Not provided" since the dataset only contains report text.
                output = graph.invoke(
                    {
                        "medical_input": sample["clinical_text"],
                        "health_data": "Not provided",
                ...
                results.append(
                    {
                        "source_text": sample["clinical_text"],
                        "reference_question": sample.get("reference_question", ""),
                        "understanding_questions": output.get(
                            "understanding_questions", ""
                        ),
                        "treatment_questions": output.get("treatment_questions", ""),
                        "lifestyle_questions": output.get("lifestyle_questions", ""),
                    }
                )
                ...
                results.append(
                    {
                        "source_text": sample["clinical_text"],
                        "reference_question": sample.get("reference_question", ""),
                        "understanding_questions": f"[ERROR: {e}]",
                        "treatment_questions": f"[ERROR: {e}]",
                        "lifestyle_questions": f"[ERROR: {e}]",
                    }
                )
        ...
        valid_results = [
            r for r in results if not r["understanding_questions"].startswith("[ERROR")
        ]
        ...
        logger.info(
            "  Avg Faithfulness: %s",
            metrics.get("avg_faithfulness_score", "N/A"),
        )
        logger.info(
            "  Faithfulness Pass Rate: %s",
            metrics.get("faithfulness_pass_rate", "N/A"),
        )
        logger.info(
            "  Understanding Faithfulness: %s",
            metrics.get("avg_understanding_faithfulness", "N/A"),
        )
        logger.info(
            "  Treatment Faithfulness: %s",
            metrics.get("avg_treatment_faithfulness", "N/A"),
        )
        logger.info(
            "  Lifestyle Faithfulness: %s",
            metrics.get("avg_lifestyle_faithfulness", "N/A"),
        )
        logger.info("-" * 60)
