...
1. **Ingest**: Seamlessly sync wearable data (Heart Rate, Steps) via Health Connect and upload diagnostic PDFs or medical images.
...
- **Framework**: Flutter (targeting Android primarily)
- **Design System**: Material 3, Google Fonts (Outfit), subtle Noise Background, customized cards with elevated shadows.
- **Navigation**: `go_router` for structured routing.
- **State Management**: Strict native MVVM (Model-View-ViewModel) using `ChangeNotifier` and `ValueNotifier` (No Riverpod, Bloc, or GetX).
